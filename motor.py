...
class MotorController2(baserobot.Controller):
    ...
    def handle_command(self):
        ...
        while True:
            ...
            # cmdの要素の数は2または4である
            assert len(cmd) == 2 or len(cmd) == 4, 'コマンドのフォーマットが違います'
            ...
            # 指定した速度にする
            self.set_v(v_right, v_left)
    
    def set_v(self, v_right, v_left):
        # 現在の速度と指定した速度の差
        dv_right = v_right - self._v_right
        dv_left = v_left - self._v_left
        # 加速する
        self.accelerate(dv_right, dv_left)

    def accelerate(self, dv_right, dv_left):
        # 左右のモータを別のスレッドで実行する
        t_right = threading.Thread(target = self._accelerate_right, args=(dv_right,))
        t_left = threading.Thread(target = self._accelerate_left, args=(dv_left,))
        t_right.start()
        t_left.start()
